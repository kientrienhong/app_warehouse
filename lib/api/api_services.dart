...
class ApiServices {
  ...
  static Future<dynamic> loadListStorage(int page, int size, String jwt,
    ...
    try {
      ...
      if (isPrice != null)
        return Dio().get(
            'https://wafayuapi.ml/api/v1/storages?Address=$address&IsSortedPrice=$isPrice&page=$page&size=$size',
            options: Options(headers: {
              'Authorization': 'bearer ' + jwt,
              'Content-Type': "application/json",
              'Accept': 'application/json',
      ...
      if (isRating != null)
        return Dio().get(
            'https://wafayuapi.ml/api/v1/storages?Address=$address&IsSortedRating=$isRating&page=$page&size=$size',
            options: Options(headers: {
              'Authorization': 'bearer ' + jwt,
              'Content-Type': "application/json",
              'Accept': 'application/json',
